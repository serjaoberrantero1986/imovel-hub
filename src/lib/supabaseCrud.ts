  ...
  try {
    ...
    if (dbConversations && dbConversations.length > 0) {
      for (const c of dbConversations) {
        ...
        const counterpartDeleted = !otherUserId;
        const coverImage = prop?.property_images?.find((img: any) => img.is_cover)?.url || prop?.property_images?.[0]?.url;
        ...
        const msgs: Message[] = visibleMessages
          .map((m: any) => {
            ...
            const senderDeleted = !m.sender_id;
            const senderProfile = profilesMap[m.sender_id];

            return {
              id: m.id,
              conversationId: m.conversation_id,
              senderId: isLeadOrigin ? `lead-sender-${c.id}` : (m.sender_id || `deleted-user-${c.id}`),
              senderName: isMine ? 'Você' : (senderDeleted ? 'Usuário excluído' : isLeadOrigin ? leadName : (otherProfile?.name || 'Usuário')),
              senderAvatar: isMine 
        ...
        conversationsList.push({
          ...
          otherUser: isLeadPortalConv ? {
            ...
            id: otherProfile?.id || otherUserId || `deleted-user-${c.id}`,
            name: counterpartDeleted ? 'Usuário excluído' : (otherProfile?.name || (isUserBuyer ? 'Corretor do Imóvel' : 'Comprador Interessado')),
            email: counterpartDeleted ? '' : (otherProfile?.email || 'contato@imovelhub.com.br'),
