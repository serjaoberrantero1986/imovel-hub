...
export const CrmNewLeadModal: React.FC<CrmNewLeadModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await onAddLead({
      ...
      status,
      priority,
      message: notes || 'Lead cadastrado pela equipe comercial.',
      propertyId: chosenProp?.id || 'lead-geral',
      propertyTitle: chosenProp?.title || 'Busca Personalizada',
      propertyPrice: chosenProp?.price || Number(budget) || 0,
      propertyImage: chosenProp?.media[0]?.thumbnailUrl || chosenProp?.media[0]?.url,
